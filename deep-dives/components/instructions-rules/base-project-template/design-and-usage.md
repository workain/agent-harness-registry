...
dialog for the folder "or for a parent directory whose trust extends to it", while a `-p`/SDK
session never shows the dialog and treats the folder as trusted. That parent-directory escape does
NOT cover a freshly `git init`-ed project: the permissions page it links to (§ Workspace trust,
fetched 2026-09-24) says trust covers subdirectories "apart from a git repository nested inside it,
such as a clone", and that "the parent-folder column doesn't apply inside a nested repository".
And in neither case does any of this reach a human typing `git commit` in a terminal — it is a
`PreToolUse` hook on the agent's tool calls, and `git init` installs no git hook.

That design choice is grounded in a specific,
...
| Memory / repeated context | You're explaining the same thing twice — the same fact re-taught across sessions, or the same mistake hit twice | Curate the existing auto-memory per `.claude/memory-notes.md` (split an overloaded entry, promote a real one to `LESSONS.md`, delete what's stale) — not new memory infrastructure |
