...
Пятнадцать файлов (четырнадцать плюс симлинк `AGENTS.md`), ни одного лишнего: каждый отвечает
...
задним числом. **Сверх дня 0** — лежит заранее и УЖЕ включено, но это не день-0-практика:
если названного действия или сигнала у вас нет, удалите — вместе со строкой-указателем на
это в `CLAUDE.md`. Удалить половину (файл без указателя или указатель без файла) — хуже,
чем не удалять.
...
| `doc/adr/` | Почему — для решений, которые переживут своих авторов. Пустая папка с форматом и порогом: записей здесь нет | сверх дня 0 |
...
гейта и хука — в этих русских текстах; в двух общих файлах слово одно.
