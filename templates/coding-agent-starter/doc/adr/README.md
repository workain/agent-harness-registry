...
Удаляя её, удалите и относящуюся к ней **часть** строки-указателя в `CLAUDE.md` — именно
часть, не строку. Было:
...
Указатель на `DECISIONS.md` остаётся: этот файл день 0 и никуда не уходит. Удалить строку
целиком значит снять живой указатель на живой файл.
